...
{
    std::cout << "YOLOV8::init Debug Start" << std::endl;
    if (trtFile.empty())
    {
        return false;
    }
    ...
    std::cout << "	m_output_dims.nbDims: " << m_output_dims.nbDims << std::endl;
    ...
    std::cout << "	m_output_dims.d[0]: " << m_output_dims.d[0] << std::endl; 
    m_output_area = 1;
    for (int i = 1; i < m_output_dims.nbDims; i++)
    {
        if (m_output_dims.d[i] != 0)
        {
            ...
            std::cout << "	m_output_dims.d[" << i << "]: " << m_output_dims.d[i] << std::endl; 
        }
    }
    ...
    cv::Mat src2dst = (cv::Mat_<float>(2, 3) 
                        << scale, 0.f, (-scale * m_param.src_w + m_param.dst_w + scale - 1) * 0.5,
                            0.f, scale, (-scale * m_param.src_h + m_param.dst_h + scale - 1) * 0.5);
    
    std::cout << "	src2dst.ptr<float>[0][0]: " << src2dst.ptr<float>(0)[0] << std::endl;
    std::cout << "	src2dst.ptr<float>[0][1]: " << src2dst.ptr<float>(0)[1] << std::endl;
    std::cout << "	src2dst.ptr<float>[0][2]: " << src2dst.ptr<float>(0)[2] << std::endl;
    std::cout << "	src2dst.ptr<float>[1][0]: " << src2dst.ptr<float>(1)[0] << std::endl;
    std::cout << "	src2dst.ptr<float>[1][1]: " << src2dst.ptr<float>(1)[1] << std::endl;
    std::cout << "	src2dst.ptr<float>[1][2]: " << src2dst.ptr<float>(1)[2] << std::endl;
    ...
    std::cout << "	dst2src.ptr<float>(0)[0]: " << dst2src.ptr<float>(0)[0] << std::endl;
    std::cout << "	dst2src.ptr<float>(0)[1]: " << dst2src.ptr<float>(0)[1] << std::endl;
    std::cout << "	dst2src.ptr<float>(0)[2]: " << dst2src.ptr<float>(0)[2] << std::endl;
    std::cout << "	dst2src.ptr<float>(1)[0]: " << dst2src.ptr<float>(1)[0] << std::endl;
    std::cout << "	dst2src.ptr<float>(1)[1]: " << dst2src.ptr<float>(1)[0] << std::endl;
    std::cout << "	dst2src.ptr<float>(1)[2]: " << dst2src.ptr<float>(1)[2] << std::endl;
    std::cout << "YOLOV8::init Debug Ends" << std::endl;
    return true;
}
...
void YOLOV8::postprocess(const std::vector<cv::Mat>& imgsBatch)
{
	...
	// [N,8400,84] --> [N,300*7+1] 保留的检测框数量300
	// dst的第一个元素用于存储有效框的数量
    ...
    nmsDeviceV2(m_param, m_output_objects_device, m_output_objects_width, m_param.topK, m_param.topK * m_output_objects_width + 1, 
				m_output_idx_device, m_output_conf_device);
    CHECK(cudaMemcpy(m_output_objects_host, m_output_objects_device, m_param.batch_size * sizeof(float) * (1 + 7 * m_param.topK), 
